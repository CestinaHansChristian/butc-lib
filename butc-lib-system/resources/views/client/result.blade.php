            ...
            <div class="search-btn-wrapper absolute top-0 h-full w-full grid place-items-center ">
                <div class="wrapper space-y-1 relative">
                    ...
                    @if ($values && $values->count() > 0)
                        <div class="content-wrapper max-h-48 bg-slate-200 p-2 rounded-md overflow-y-scroll space-y-2 absolute w-full">
                            {{-- card container content --}}
                            @foreach ($values as $r)
                            <div class="card grid md:grid-cols-5 h-full p-2 rounded-md bg-slate-300">
                                <div class="grid-container grid grid-rows-3 text-sm md:text-lg  md:col-span-4">
                                ...
                                <form action="{{ route('guest.show', $r->id) }}" method="get" class="download-option-wrapper bg-red-300 rounded-xl cursor-pointer hover:bg-red-400">
                                    <button  type="submit" class="h-full w-full grid place-content-center items-center">
